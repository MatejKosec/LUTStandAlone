...
CTrapezoidalMap::CTrapezoidalMap(vector< su2double > x_samples,
		vector< su2double > y_samples, vector< vector<int> > unique_edges) {
	...
	unique(Unique_X_Bands.begin(),Unique_X_Bands.end());
    ...
    Y_Value_of_Edge_Within_Band_And_Index =
...
CLookUpTable::CLookUpTable(string Filename) {
	...
	rhoe_map[0] = CTrapezoidalMap(ThermoTables_Density[0],ThermoTables_StaticEnergy[0], Table_Zone_Edges[0]);
...
void CLookUpTable::Search_Linear_Skewed_Table(su2double x, su2double P,
	vector< su2double > *ThermoTables_X) {
...
Search_j_for_Y_given_i(rho, e, ThermoTables_Density, ThermoTables_StaticEnergy);
...
Search_NonEquispaced_Rho_Index(rho);
Search_j_for_Y_given_i(rho, T, ThermoTables_Density, ThermoTables_Temperature);
